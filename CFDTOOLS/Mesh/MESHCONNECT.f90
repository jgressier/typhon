...
if (iverb > 0) then
  call cfd_print("  > "//trim(strof(nface))//" created faces")
  call cfd_print("  > "//trim(strof(count(umesh%facecell%fils(1:nface,2)==0)))//" external faces")
endif
...
do ib = 1, umesh%nboco
  ...
  if (iverb > 0) &
    call cfd_print("  . linking boundary condition marks"//strofr(ib,3)//"/"//trim(strof(umesh%nboco))// &
