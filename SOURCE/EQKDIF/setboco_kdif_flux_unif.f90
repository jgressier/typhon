...
!
!------------------------------------------------------------------------------!
subroutine setboco_kdif_flux_unif(ustboco, ustdom, champ, flux, defsolver, wallflux)
...
real(krp)        :: wallflux         ! flux condition limite
...
do ifb = 1, ustboco%nface
  ...
  do ip = 1, champ%nscal
    champ%etatprim%tabscal(ip)%scal(ighost) = champ%etatprim%tabscal(ip)%scal(ic) - wallflux*d/conduct
  enddo

enddo

endsubroutine setboco_kdif_flux_unif
