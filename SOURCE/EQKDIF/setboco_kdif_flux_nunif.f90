!------------------------------------------------------------------------------!
! Procedure : setboco_kdif_flux_unif      Auteur : J. Gressier/E. Radenac
!                                         Date   : Juin 2004
...
!
!------------------------------------------------------------------------------!
subroutine setboco_kdif_flux_nunif(ustboco, ustdom, champ, flux, defsolver, wallflux)
...
real(krp), dimension(ustboco%nface) &
                 :: wallflux         ! flux condition limite
...
do ifb = 1, ustboco%nface
  ...
  ! Flux limite
  flux(ifb) = wallflux(ifb)

  ! Calcul approché de la température du point fictif pour calcul des gradients
  do ip = 1, champ%nscal
    champ%etatprim%tabscal(ip)%scal(ighost) = champ%etatprim%tabscal(ip)%scal(ic) - wallflux(ifb)*d/conduct
  enddo

enddo

endsubroutine setboco_kdif_flux_nunif
