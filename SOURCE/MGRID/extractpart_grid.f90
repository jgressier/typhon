...
type(st_ustboco), pointer &
                      :: boco(:)
integer(kip)          :: i, if, ic, ic1, ic2, iv, ivf, if2, nicl, nf, ib, new_ib
...
do ib = 1, fullgrid%umesh%nboco
  ...
  if (nf /= 0) then
    ...
    do if = 1, fullgrid%umesh%boco(ib)%nface
      if (new_iface(fullgrid%umesh%boco(ib)%iface(if)) /= 0) then
        if2 = if2 + 1
        boco(new_ib)%iface(if2) = new_iface(fullgrid%umesh%boco(ib)%iface(if))
      endif
    enddo
  endif
enddo
