...
iv0 = umesh%nvtex
iv  = umesh%nvtex + umesh%ncell_int * (defspat%svm%intnode -3)        ! index offset
do ic = 1, umesh%cellvtex%elem(ielem)%nelem
...
ielemquad = 1
ielemhexa = 2
nquad = (defspat%svm%cv_split -1) * umesh%cellvtex%elem(ielem)%nelem               ! define number of QUAD and PENT
...
do ic=1,umesh%ncell_int 
  icnq = (ic-1) * (defspat%svm%cv_split -1)
  icnh = (ic-1)
...
newmesh%cellvtex%elem(ielemquad)%ielem(icnq+4) = (ic-1) * defspat%svm%cv_split + 4
newmesh%cellvtex%elem(ielemquad)%ielem(icnq+5) = (ic-1) * defspat%svm%cv_split + 5
newmesh%cellvtex%elem(ielemquad)%ielem(icnq+6) = (ic-1) * defspat%svm%cv_split + 6
newmesh%cellvtex%elem(ielemquad)%ielem(icnq+7) = (ic-1) * defspat%svm%cv_split + 7
newmesh%cellvtex%elem(ielemquad)%ielem(icnq+8) = (ic-1) * defspat%svm%cv_split + 8
newmesh%cellvtex%elem(ielemquad)%ielem(icnq+9) = (ic-1) * defspat%svm%cv_split + 9
newmesh%cellvtex%elem(ielemhexa)%ielem(icnh+1) = (ic-1) * defspat%svm%cv_split + 10
enddo

call print_info(20, "    . creating"//strof(nquad + nhexa,7)//" CV cells")
...
do ic = 1, umesh%ncell_int
  ...
  intv(1)    = umesh%nvtex + (ic-1) * (defspat%svm%intnode - 3) + 1                         ! 3 first internal vertices
  intv(2)    = umesh%nvtex + (ic-1) * (defspat%svm%intnode - 3) + 2
  intv(3)    = umesh%nvtex + (ic-1) * (defspat%svm%intnode - 3) + 3
  intv(4)    = umesh%nvtex + umesh%ncell_int * (defspat%svm%intnode -3) + (ic-1) * (defspat%svm%intnode - 3) + 1! 3 other internal vertices
  intv(5)    = umesh%nvtex + umesh%ncell_int * (defspat%svm%intnode -3) + (ic-1) * (defspat%svm%intnode - 3) + 2
  intv(6)    = umesh%nvtex + umesh%ncell_int * (defspat%svm%intnode -3) + (ic-1) * (defspat%svm%intnode - 3) + 3
